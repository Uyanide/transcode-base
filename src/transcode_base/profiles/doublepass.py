"""Typed profile for the 2-pass(Doublepass) subsystem.

Mirrors `doublepass.toml`:

    [video.<encoder>]    flat key/value table, each becomes `-key value` on the CLI
    [audio.<encoder>]    same shape, used for `-c:a`

The 2-pass specific params should NOT be included in the profile, and should be
handled by the consumer. (-an, -pass 1/2, -passlogfile, etc.)
"""
